...
public final class ScreenSnapshots extends Application {
    ...
    private void planSteps() {
        ...
        steps.add(this::runToWeather);
        steps.add(() -> shoot("11-weather"));
    ...
    /**
     * Winds an Ancient Egypt level on to the middle of a weather.
     */
    private void runToWeather() {
        var session = ((ir.sharif.pvz.controller.GameMenuController) app.currentController())
                .getSession();
        double period = ir.sharif.pvz.model.game.Weather.PERIOD_SECONDS;
        double wanted = ir.sharif.pvz.model.game.Weather.CROSSING_SECONDS / 2;
